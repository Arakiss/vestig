...
`batchSize` controls when Vestig starts a send. `maxBufferSize` controls how many additional entries can wait in memory while that send is still in flight. The default is `batchSize * 2`; increase it for bursty cron, queue, or edge workloads where a destination may briefly be slower than the producer.
