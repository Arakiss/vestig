...
export const GET = withVestig(async (request, { log }) => {
  // The handler runs inside the request context, so this query is logged
  // with the same requestId and traceId as everything else in the request
...
In a Server Component, read the context that the middleware already
established instead of opening a new one:
...
export default async function Page() {
  ...
  const { requestId, traceId } = await getRequestContext()

  const users = await prisma.user.findMany()
  log.info('Rendering users', { count: users.length, requestId, traceId })
