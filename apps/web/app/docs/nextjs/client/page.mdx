export const metadata = {
  ...
  description: 'React hooks and providers for client-side logging with Vestig. useLogger hook and VestigProvider.',
}
...
interface VestigProviderProps {
  ...
  // Remote client log endpoint.
  // Defaults to '/api/vestig'; set to false for correlation-only mode.
...
Use `endpoint={false}` when you want `VestigProvider`, `useLogger()`, and
`useCorrelationContext()` without remote browser log delivery.

```typescript
<VestigProvider initialContext={ctx} endpoint={false}>
  {children}
</VestigProvider>
```
